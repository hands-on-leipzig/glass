...
import VenueDetailMap from './VenueDetailMap.vue'
...
const programKey = computed(() =>
    props.venue ? venueProgramKey(props.venue) : 'other',
)

const programLabel = computed(() => {
  const key = programKey.value
  if (!props.venue || key === 'other') return ''
  return t(`venues.offerShort.${key}`)
})

const programChipStyle = computed(() => ({
      ...
      >
        ...
        >
          <header class="venue-detail-modal-head">
            <div class="venue-detail-modal-heading">
              <h2 class="venue-detail-modal-title">{{ title }}</h2>
              <span
                  v-if="programLabel"
                  class="venue-detail-program-chip"
                  :style="programChipStyle"
              >{{ programLabel }}</span>
            </div>
            <button
            ...
            <aside class="venue-detail-modal-side">
              ...
              <div class="venue-detail-links">
                <a
                    ...
                    target="_blank"
                >
                  {{ t('venues.detailWebsite') }}
                  <i aria-hidden="true" class="bi bi-box-arrow-up-right"/>
                </a>
                <slot name="links" :venue="venue" />
              </div>
            </aside>
          </div>
        </div>
